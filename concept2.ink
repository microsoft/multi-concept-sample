...
graph (input: ObservableState) {
    concept MoveToTarget(input): SimAction {
        curriculum {
            ...
            goal (State: ObservableState) {
                ...
                drive `To target position`: #modifying the target position
                    [State.ball_x, State.ball_y] in Goal.Sphere([State.target_x, State.target_y], CloseEnough)
            }
            ...
            lesson `Randomize Start` {
                ...
                scenario {
                    initial_x: number<-RadiusOfPlate * 0.02 .. RadiusOfPlate * 0.02>, # smaller initial radius from0.5 to 0.02
                    initial_y: number<-RadiusOfPlate * 0.02 .. RadiusOfPlate * 0.02>,

                    initial_vel_x: number<-MaxVelocity * 0.02 .. MaxVelocity * 0.02>, # keeping same velocity (may reduce later)
                    initial_vel_y: number<-MaxVelocity * 0.02 .. MaxVelocity * 0.02>,
                    
                    initial_pitch: number<-0.2 .. 0.2>,
                    initial_roll: number<-0.2 .. 0.2>,

                    target_x: number<-RadiusOfPlate * 0.5 .. RadiusOfPlate * 0.5>,
                    target_y: number<-RadiusOfPlate * 0.5 .. RadiusOfPlate * 0.5>,
                }
            }
        }
    }
